...
import {
  ...
  getTableOrderStatusLogsByUser,
} from '@/lib/db';
import { verifySessionToken } from '@/lib/auth';

/** GET /api/admin/stats/table-logs?userId=xxx — سجلات تغيير حالة طلبات الطاولة (باقة البزنس فقط) */
export async function GET(request: NextRequest) {
  try {
    ...
    const allLogs = await getTableOrderStatusLogsByUser(adminId, userId);
