...
function AdminPageContent() {
  ...
  // نشاط العملاء (Basic+): القائمة تجلب اسم+رقم+عداد+عنوان؛ العنوان يُعرض في التفاصيل فقط
  type CustomerRow = { adminId: string; phone: string; name: string; orderCount: number; address?: string };
  ...
  const [editingOrderDetails, setEditingOrderDetails] = useState<{ orderId: string; customerName: string; customerPhone: string; customerAddress: string } | null>(null);
  ...
  const handleSaveOrderDetails = async (updates: { customerName?: string; customerPhone?: string; customerAddress?: string }) => {
    if (!editingOrderDetails) return;
    setSavingOrderDetails(true);
    try {
      const res = await fetch(`/api/orders/${editingOrderDetails.orderId}/details`, {
        method: 'PATCH',
        headers: getAuthHeaders(),
      ...
      if (res.ok && data.order) {
        setOrders(prev => prev.map(o => o.id === editingOrderDetails.orderId ? { ...o, ...data.order } : o));
        setEditingOrderDetails(null);
      } else {
        alert(data.error || 'فشل تحديث بيانات الطلب');
      }
    } catch {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'customerActivity' && (
          <div className="w-full max-w-4xl mx-auto p-4 space-y-6" dir="rtl">
            <h2 className="text-2xl font-bold text-gray-800">نشاط عملاء التوصيل</h2>
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
              <input
                type="search"
                ...
                <div ref={customerDetailsRef} className="pt-6 border-t border-gray-200">
                  {selectedCustomer && (
                    <div className="bg-white rounded-xl shadow border border-gray-100 p-6 space-y-4">
                      ...
                      <p className="text-lg font-bold text-gray-800">{(selectedCustomerDetail ?? selectedCustomer).name || '—'} <span className="text-gray-600 font-medium">({(() => { const c = (selectedCustomerDetail ?? selectedCustomer).orderCount ?? 0; if (c === 1) return 'طلب واحد'; if (c === 2) return 'طلبان'; if (c >= 3 && c <= 10) return `${c} طلبات`; return `${c} طلباً`; })()})</span></p>
                      <div className="flex flex-col gap-2 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-800">{(selectedCustomerDetail ?? selectedCustomer)?.address || '—'}</span>
                        </div>
                      </div>
                      {isPlanActive('business') && (
...
? 'bg-gradient-to-r from-slate-600 to-slate-800 hover:from-slate-700 hover:to-slate-900'
        ...
        {activeTab === 'orders' && (
          <div className="bg-white p-8 rounded-lg shadow-md">
                    ...
                    <div key={order.id} className="border-2 border-emerald-300 rounded-xl p-4 hover:shadow-lg transition-shadow relative bg-emerald-50">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-[auto_1fr_auto_150px] gap-3 md:gap-4">
                        ...
                        <div className="col-span-2 md:col-span-1 md:order-2 flex items-center justify-start">
                          <div className="flex flex-wrap items-center justify-center gap-1 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 w-full max-w-full">
                            {order.orderType === 'whatsapp' ? (
                              <>
                                <svg className="w-5 h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                                </svg>
                                <button type="button" onClick={() => setEditingOrderDetails({ orderId: order.id, customerName: order.customerName ?? '', customerPhone: order.customerPhone ?? '', customerAddress: order.customerAddress ?? '' })} className="text-sm font-bold text-emerald-700 hover:underline break-words text-right">{order.customerPhone?.trim() ? order.customerPhone : 'الرقم'}</button>
                                <span className="text-emerald-400">•</span>
                                <button type="button" onClick={() => setEditingOrderDetails({ orderId: order.id, customerName: order.customerName ?? '', customerPhone: order.customerPhone ?? '', customerAddress: order.customerAddress ?? '' })} className="text-sm font-bold text-emerald-700 hover:underline break-words text-right">{order.customerName?.trim() ? order.customerName : 'الاسم'}</button>
                                <span className="text-emerald-400">•</span>
                                <button type="button" onClick={() => setEditingOrderDetails({ orderId: order.id, customerName: order.customerName ?? '', customerPhone: order.customerPhone ?? '', customerAddress: order.customerAddress ?? '' })} className="text-sm font-bold text-emerald-700 hover:underline break-words text-right">العنوان</button>
                              </>
                            ) : (
                              <>
                                <svg className="w-5 h-5 text-emerald-900 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                                {order.customerPhone?.trim() && <span className="text-sm font-bold text-emerald-900 break-words">{order.customerPhone}</span>}
                                {order.customerPhone?.trim() && (order.customerName?.trim() || true) && <span className="text-emerald-400">•</span>}
                                <button type="button" onClick={() => setEditingOrderDetails({ orderId: order.id, customerName: order.customerName ?? '', customerPhone: order.customerPhone ?? '', customerAddress: order.customerAddress ?? '' })} className="text-sm font-bold text-emerald-700 hover:underline break-words text-right">{order.customerName?.trim() ? order.customerName : 'الاسم'}</button>
                                <span className="text-emerald-400">•</span>
                                <button type="button" onClick={() => setEditingOrderDetails({ orderId: order.id, customerName: order.customerName ?? '', customerPhone: order.customerPhone ?? '', customerAddress: order.customerAddress ?? '' })} className="text-sm font-bold text-emerald-700 hover:underline break-words text-right">العنوان</button>
                              </>
            ...
            {/* نافذة تعديل بيانات عميل الطلب (اسم، رقم، عنوان) */}
            {editingOrderDetails && (() => {
              const orderBeingEdited = orders.find(o => o.id === editingOrderDetails.orderId);
              const isWebsiteOrder = orderBeingEdited?.orderType === 'website';
              const payload = isWebsiteOrder
                ? { customerName: editingOrderDetails.customerName, customerAddress: editingOrderDetails.customerAddress }
                : { customerName: editingOrderDetails.customerName, customerPhone: editingOrderDetails.customerPhone, customerAddress: editingOrderDetails.customerAddress };
              return (
              <>
                <div className="fixed inset-0 bg-black/50 z-[70]" onClick={() => setEditingOrderDetails(null)} />
                <div className="fixed inset-0 z-[71] flex items-center justify-center p-4" dir="rtl">
                  <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                    <h3 className="text-xl font-bold text-gray-800 mb-4">بيانات العميل</h3>
                    <div className="space-y-3 mb-6">
                      <div>
                        <label className="block text-gray-700 font-medium mb-1">الاسم</label>
                        <input
                          type="text"
                          value={editingOrderDetails.customerName}
                          onChange={e => setEditingOrderDetails(prev => prev ? { ...prev, customerName: e.target.value } : null)}
                      ...
                      <div>
                        ...
                        <input
                          type="tel"
                          value={editingOrderDetails.customerPhone}
                          onChange={e => !isWebsiteOrder && setEditingOrderDetails(prev => prev ? { ...prev, customerPhone: e.target.value } : null)}
                          readOnly={isWebsiteOrder}
                          disabled={isWebsiteOrder}
                          className={`w-full px-3 py-2 border rounded-lg text-gray-800 ${isWebsiteOrder ? 'border-gray-200 bg-gray-100 cursor-not-allowed' : 'border-gray-300'}`}
                          placeholder="رقم الهاتف"
                        />
                      </div>
                      <div>
                        <label className="block text-gray-700 font-medium mb-1">العنوان</label>
                        <input
                          type="text"
                          value={editingOrderDetails.customerAddress}
                          onChange={e => setEditingOrderDetails(prev => prev ? { ...prev, customerAddress: e.target.value } : null)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-800"
                          placeholder="العنوان"
                        />
                      </div>
                    ...
                    <div className="flex gap-2 justify-end">
                      <button type="button" onClick={() => setEditingOrderDetails(null)} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg font-medium">إلغاء</button>
                      <button type="button" disabled={savingOrderDetails} onClick={() => handleSaveOrderDetails(payload)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium disabled:opacity-70">{savingOrderDetails ? 'جاري الحفظ...' : 'حفظ'}</button>
                    </div>
                  </div>
                </div>
              </>
